...
/**
 * Get document id from (request) id which can be either ObjectID or String
 */
function docId(id) {
 var oid_or_string;
 try {
   oid_or_string = new BSON.ObjectID(id);	
 } catch(error) {
   oid_or_string = id;
 }
...
app.get('/:db/:collection/:id?', function(req, res) { 
  ...
  db.open(function(err,db) {
    ...
    if(config.db.authenticationenabled === false) {
      collectionFunction();      	
    } else {
      db.authenticate(config.db.username, config.db.password,collectionFunction());
    }
  });
});
...
app.put('/:db/:collection/:id', function(req, res) {
  var spec = {'_id': docId(req.params.id)};
